...
const formatPlayers = (players) => {
	...
	players.forEach((player) => {
		...
		// AI-based fake detection heuristics - refined to reduce false positives
		let isFake = false;
		let fakeScore = 0;
		const reasons = [];

		// High ping (likely fake or bot) - increased threshold
		if (player.ping > 1000) {
			fakeScore += 2;
			reasons.push('Very high ping');
		} else if (player.ping > 800) {
			fakeScore += 1;
			reasons.push('High ping');
		}

		// No Steam ID (many real players have Steam linked) - reduced weight
		if (!socials.steam) {
			fakeScore += 1;
			reasons.push('No Steam ID');
		}

		// Suspicious name patterns - more specific
		const name = player.name || '';
		if (/bot|fake|test|spam|hack/i.test(name)) {
			fakeScore += 2;
			reasons.push('Suspicious keywords in name');
		} else if (name.length < 2 || /^\d{5,}$/.test(name)) {
			fakeScore += 1;
			reasons.push('Suspicious name pattern');
		}

		// ID consistency check (IDs should be sequential-ish, but allow some gaps)
		if (player.id < 0 || player.id > 65535) { // FiveM IDs are typically 0-65535
			fakeScore += 2;
			reasons.push('Invalid ID range');
		}

		// Additional heuristic: If ping is 0 and no socials, likely fake
		if (player.ping === 0 && !socials.steam && !socials.discord) {
			fakeScore += 2;
			reasons.push('Zero ping with no socials');
		}

		// Only mark as fake if score is high enough (multiple indicators)
		if (fakeScore >= 3) {
			isFake = true;
		}
...
export const renderPlayers = (players, search = false) => {
	...
	players.forEach((player) => {
		...
		if (player.isFake) {
			...
			fakeBadge.textContent = 'FAKE';
			fakeBadge.className = 'fake-badge';
			fakeBadge.title = `Fake reasons: ${player.fakeReasons.join(', ')}`;
			name.appendChild(fakeBadge);

			const fakeIcon = document.createElement('img');
			fakeIcon.src = 'img/question-mark.svg'; // Use existing question-mark icon as warning
			fakeIcon.alt = 'Potential Fake Player';
			fakeIcon.title = `Fake reasons: ${player.fakeReasons.join(', ')}`;
			fakeIcon.style.width = '16px';
			fakeIcon.style.height = '16px';
			fakeIcon.style.marginLeft = '5px';
			name.appendChild(fakeIcon);
		}
