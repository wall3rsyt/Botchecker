...
const formatPlayers = (players) => {
	...
	players.forEach((player) => {
		...
		// AI-based fake detection heuristics
		let isFake = false;
		const reasons = [];

		// High ping (likely fake or bot)
		if (player.ping > 500) {
			isFake = true;
			reasons.push('High ping');
		}

		// No Steam ID (many real players have Steam linked)
		if (!socials.steam) {
			isFake = true;
		...
		if (name.length < 3 || /^\d+$/.test(name) || /bot|fake|test/i.test(name)) {
			isFake = true;
			reasons.push('Suspicious name');
		}

		// ID consistency check (IDs should be sequential-ish, but allow some gaps)
		// This is basic; in a real AI, use ML for patterns
