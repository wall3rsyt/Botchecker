...
async function fetchServerDetails(endpoint) {
  try {
    const res = await fetch(`${FIVEM_API}/${endpoint}`);
...
async function loadServers(query="") {
  ...
  if(query){
    filtered = serverList.filter(s=> (s.EndPoint||"").toLowerCase().includes(query.toLowerCase()) || (s.Data?.hostname||"").toLowerCase().includes(query.toLowerCase()));
  }
